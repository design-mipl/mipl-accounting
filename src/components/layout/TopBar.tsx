import { Search, Bell, HelpCircle, Settings, LogOut, Lock } from 'lucide-react'
        ...
        <div className="flex items-center gap-1 ml-auto">
          ...
          <div className="flex items-center gap-2 ml-2 pl-2 border-l border-gray-200">
            <div className="w-7 h-7 rounded-full bg-indigo-600 flex items-center justify-center text-white text-[11px] font-bold">
              {user?.firstName && user?.lastName ? `${user.firstName[0]}${user.lastName[0]}`.toUpperCase() : user?.email ? user.email[0].toUpperCase() : 'U'}
            </div>
            {user && (
